...
std::vector<MyString> MyString::splits(const MyString &ftm) const
{
    ...
    int smartmode = 0;
    char lastflagchar = '\0';
    while (isCouple(ftm[smartmode], ftm[ftm.size() - 1 - smartmode]) && smartmode < static_cast<int>((ftm.size() / 2)))
        smartmode++;
    ...
    for (size_t i = 0; i < size(); i++)
    {
        if ((flag = ftm.find(at(i))) != std::string::npos)
        {
            ...
            { //出现需要配对的情况
                if (ftm[flag] == lastflagchar && ftm[flag] == ftm[ftm.size() - flag - 1] && deep == 1)
                {
                    flag = ftm.size() - flag - 1;
                }
                if (static_cast<int>(flag) < smartmode)
                {
                    if (deep == 0) //如果现在检测到成对的第一个字符且深度为零,清空前面的字符作为一个整体
                    {
                        lastflagchar = ftm[flag];
                        if (((i - lastindex) < 2) || i == 0)
                        { //检测到两个相邻的标致
                            lastindex = i;
                        }
                    ...
                    if (ftm[flag] == lastflagchar)
                        deep++;
                }
                else if (isCouple(ftm[flag], lastflagchar))
...
bool MyString::isCouple(const char a, const char b) const
{
    return ((a == '\'' && b == '\'') ||
            (a == '\"' && b == '\"') ||
            (a == '(' && b == ')') ||
...
// bool MyString::isCouple(int a, int b)
// {
//     return true;
// }
